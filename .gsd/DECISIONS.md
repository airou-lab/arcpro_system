...
| D006 | Manually reconstruct policy MLP architecture in verification script. | Avoids direct dependency on the full `rsl_rl` training runner configuration, ensuring `verify_policy.py` can load weights into a compatible torch module without access to the original training script's hyperparameter classes. | 2026-03-16 |
